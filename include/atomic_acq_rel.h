...
struct atomic_acq_rel final
{
    ...
    void store(T desired) noexcept { m_value.store(std::forward<T>(desired), std::memory_order::memory_order_release); }
    ...
    T load() const volatile noexcept { return std::forward<T>(m_value.load(std::memory_order::memory_order_acquire)); }
